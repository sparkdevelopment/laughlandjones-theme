...
var Contact = function () {
  function Contact() {
    ...
    $(document).ready(function () {
      ...
      if (js_cookie.get(cookieName)) {
        // have cookie
      } else {
        setTimeout(function () {
          window.Subscribe.showForm(false);
        }, 2000);
        js_cookie.set(cookieName, 'viewed');
      }
    });
  }
...
var jquery_magnificPopup = createCommonjsModule(function (module, exports) {
 ...
 * Copyright (c) 2016 Dmitry Semenov; */
   ...
   */
		var _mfpOn = function _mfpOn(name, f) {
			...
			if (mfp.st.callbacks) {
				...
				if (mfp.st.callbacks[e]) {
					mfp.st.callbacks[e].apply(mfp, $$$1.isArray(data) ? data : [data]);
				}
			}
		},
		    _getCloseBtn = function _getCloseBtn(type) {
			if (type !== _currPopupType || !mfp.currTemplate.closeBtn) {
				mfp.currTemplate.closeBtn = $$$1(mfp.st.closeMarkup.replace('%title%', mfp.st.tClose));
				_currPopupType = type;
			}
		...
		_checkInstance = function _checkInstance() {
			if (!$$$1.magnificPopup.instance) {
				/*jshint -W020 */
				mfp = new MagnificPopup();
				mfp.init();
				$$$1.magnificPopup.instance = mfp;
			}
		},
   ...
   */
    ...
    */
			open: function open(data) {
				...
				if (data.isObj === false) {
					...
					mfp.items = $$$1.isArray(data.items) ? data.items : [data.items];
				...
				mfp.st = $$$1.extend(true, {}, $$$1.magnificPopup.defaults, data);
				...
				var modules = $$$1.magnificPopup.modules;
				...
				if (mfp.fixedContentPos) {
					if (!mfp.isIE7) {
						...
						$$$1('body, html').css('overflow', 'hidden');
					}
				}
				...
				$$$1('html').css(windowStyles);

				// add everything to DOM
				mfp.bgOverlay.add(mfp.wrap).prependTo(mfp.st.prependTo || $$$1(document.body));
    ...
    */
			_close: function _close() {
				...
				if (mfp.st.autoFocusLast && mfp._lastFocusedEl) {
					$$$1(mfp._lastFocusedEl).focus(); // put tab focus back
				}
				mfp.currItem = null;
				mfp.content = null;
    ...
    */
			updateItemHTML: function updateItemHTML() {
				...
				if (!mfp.currTemplate[type]) {
					...
					if (markup) {
						mfp.currTemplate[type] = $$$1(markup);
					} else {
    ...
    */
			...
			_openClick: function _openClick(e, el, options) {
				var midClick = options.midClick !== undefined ? options.midClick : $$$1.magnificPopup.defaults.midClick;

				if (!midClick && (e.which === 2 || e.ctrlKey || e.metaKey || e.altKey || e.shiftKey)) {
					return;
				}

				var disableOn = options.disableOn !== undefined ? options.disableOn : $$$1.magnificPopup.defaults.disableOn;

				if (disableOn) {
					if ($$$1.isFunction(disableOn)) {
    ...
    */
			...
			_checkIfClose: function _checkIfClose(target) {
				...
				if ($$$1(target).hasClass(PREVENT_CLOSE_CLASS)) {
					return;
				}
				...
				if (closeOnContent && closeOnBg) {
					...
					if (!mfp.content || $$$1(target).hasClass('mfp-close') || mfp.preloader && target === mfp.preloader[0]) {
						return true;
					}

					// if click is outside the content
					if (target !== mfp.content[0] && !$$$1.contains(mfp.content[0], target)) {
						if (closeOnBg) {
							// last check, if the clicked element is in DOM, (in case it's removed onclick)
							if ($$$1.contains(document, target)) {
								return true;
							}
						}
			...
			_onFocusIn: function _onFocusIn(e) {
				if (e.target !== mfp.wrap[0] && !$$$1.contains(mfp.wrap[0], e.target)) {
					mfp._setFocus();
					return false;
				}
			},
			_parseMarkup: function _parseMarkup(template, values, item) {
				var arr;
				if (item.data) {
					values = $$$1.extend(item.data, values);
				}
				_mfpTrigger(MARKUP_PARSE_EVENT, [template, values, item]);

				$$$1.each(values, function (key, value) {
						...
						if (el.length > 0) {
							...
							if (attr === 'replaceWith') {
								...
								if (el.is('img')) {
									...
									el.replaceWith($$$1('<img>').attr('src', value).attr('class', el.attr('class')));
								}
							} else {
								el.attr(arr[1], value);
							}
   ...
   */
		$$$1.magnificPopup = {
			...
			close: function close() {
				return $$$1.magnificPopup.instance && $$$1.magnificPopup.instance.close();
			},

			registerModule: function registerModule(name, module) {
				if (module.options) {
					$$$1.magnificPopup.defaults[name] = module.options;
				}
				$$$1.extend(this.proto, module.proto);
		...
		$$$1.fn.magnificPopup = function (options) {
			_checkInstance();
     ...
     */
		...
		$$$1.magnificPopup.registerModule(INLINE_NS, {
			options: {
		...
		var AJAX_NS = 'ajax',
			...
			if (_ajaxCur) {
				$$$1(document.body).removeClass(_ajaxCur);
			}
		},
		...
		$$$1.magnificPopup.registerModule(AJAX_NS, {
			...
			proto: {
				...
				getAjax: function getAjax(item) {
					...
					if (_ajaxCur) {
						$$$1(document.body).addClass(_ajaxCur);
					}
					...
					var opts = $$$1.extend({
						...
						success: function success(data, textStatus, jqXHR) {
							...
							mfp.appendContent($$$1(temp.data), AJAX_NS);
		...
		$$$1.magnificPopup.registerModule('image', {
				...
				initImage: function initImage() {
					...
					_mfpOn(OPEN_EVENT + ns, function () {
						if (mfp.currItem.type === 'image' && imgSt.cursor) {
							$$$1(document.body).addClass(imgSt.cursor);
						}
					});

					_mfpOn(CLOSE_EVENT + ns, function () {
						if (imgSt.cursor) {
							$$$1(document.body).removeClass(imgSt.cursor);
						}
     ...
     */
				...
				getImage: function getImage(item, template) {
					...
					if (el.length) {
						...
						item.img = $$$1(img).on('load.mfploader', onLoadComplete).on('error.mfploader', onLoadError);
						img.src = item.src;
		...
		$$$1.magnificPopup.registerModule('zoom', {
			...
			proto: {
				...
				_getOffset: function _getOffset(isLarge) {
					...
					offset.top -= $$$1(window).scrollTop() - paddingTop;
		...
		$$$1.magnificPopup.registerModule(IFRAME_NS, {
			...
			proto: {
				...
				getIframe: function getIframe(item, template) {
					...
					$$$1.each(iframeSt.patterns, function () {
   ...
   */
		...
		$$$1.magnificPopup.registerModule('gallery', {
			...
			proto: {
					...
					_mfpOn('BuildControls' + ns, function () {
						if (mfp.items.length > 1 && gSt.arrows && !mfp.arrowLeft) {
							var markup = gSt.arrowMarkup,
							    arrowLeft = mfp.arrowLeft = $$$1(markup.replace(/%title%/gi, gSt.tPrev).replace(/%dir%/gi, 'left')).addClass(PREVENT_CLOSE_CLASS),
							    arrowRight = mfp.arrowRight = $$$1(markup.replace(/%title%/gi, gSt.tNext).replace(/%dir%/gi, 'right')).addClass(PREVENT_CLOSE_CLASS);
				...
				_preloadItem: function _preloadItem(index) {
					...
					if (item.type === 'image') {
						item.img = $$$1('<img class="mfp-img" />').on('load.mfploader', function () {
							item.hasSize = true;
		...
		$$$1.magnificPopup.registerModule(RETINA_NS, {
			options: {
...
var Fabric = function () {
  function Fabric() {
    ...
    $(document).ready(function () {
      $('.design-colors li').on('click', function (e) {
        e.preventDefault();
  ...
  createClass(Fabric, [{
    key: 'initBasket',
    value: function initBasket(self) {
      self.drawBasket(self);
    }
  }, {
    key: 'drawBasket',
    value: function drawBasket(self) {
    ...
    key: 'updateButtons',
    value: function updateButtons(self) {
    ...
    value: function getBasket() {
      ...
      var basketValue = js_cookie.get(this.cookieName);
    ...
    key: 'addToBasket',
    value: function addToBasket(self, fabric) {
      var basketValue = self.getBasket();
      ...
      if (!self.isInBasket(self, fabricId, variationId)) {
        ...
        js_cookie.set(this.cookieName, JSON.stringify(basketValue), { expires: 7 });
        self.drawBasket(self);
      }
    }
  }, {
    key: 'removeFromBasket',
    value: function removeFromBasket(self, fabric) {
      var basketValue = self.getBasket();
      var fabricId = parseInt($(fabric).attr('data-id'));
      var variationId = parseInt($(fabric).attr('data-variation'));
      if (self.isInBasket(self, fabricId, variationId)) {
        ...
        js_cookie.set(this.cookieName, JSON.stringify(updatedBasketValue), { expires: 7 });
        self.drawBasket(self);
    ...
    key: 'isInBasket',
    value: function isInBasket(self, fabricId, variationId) {
      var basketValue = self.getBasket();
      var found = basketValue.some(function (basketItem) {
        return basketItem.fabric === fabricId && basketItem.variation === variationId;
      });
      return found;
    }
    ...
    key: 'updateSelection',
    value: function updateSelection(self, e) {
    ...
    key: 'arrayRotate',
    value: function arrayRotate(arr, reverse, count) {
      count = count || 1;
...
var App = function () {
  function App() {
    ...
    if ($('body.single-fabric').length || $('.page-template-template-basket').length) {
      this.fabric = new Fabric();
    }
    if ($('body.post-type-archive-fabric').length) {
      this.fabric = new Fabric();
    }
